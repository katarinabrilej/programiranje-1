...
let mapi f list = 
  let rec aux acc stevec f = function
    | [] -> acc
    | x :: xs -> aux (f x stevec :: acc) (stevec+1) f xs
  in
  reverse(aux [] 0 f list)
...
let zip_enum_tlrec list1 list2 = 
  let rec aux acc stevec list1 list2 = 
    match list1, list2 with
    | [],[] -> acc 
    | x :: xs, y :: ys ->  aux((stevec, x, y) :: acc) (stevec + 1) xs ys
    | [],_ | _,[] -> failwith "Different lengths of input lists."
  in 
  reverse(aux [] 0 list1 list2)
...
let rec unzip list = 
  ...
  | [] -> ([],[])
  | (x,y) :: xs -> 
  let (levi,desni) = unzip xs in (x :: levi, y :: desni)
...
let rec fold_left_no_acc f = function
  ...
  | x :: y :: [] -> f x y 
  | x :: y :: ys ->  fold_left_no_acc f ( f x y :: ys)
