...
let max_cheese cheese_matrix = 
  ...
  let max_c = Array.length cheese_matrix.(0) in (*vzamemo prvo vrstico in pogledamo koliko elementov ima -> če je seznam prazen bo to fejlalo*)
  
  let rec max_cheese' recursive_max_cheese' (r, c) = (*row in column*)
